...
- **Desktop UI Refinement**: Desktop-only (>=1024px) visual pass in `ui_v1.css`: compact 56px header with bottom-border active nav indicator (.nav-active class via ui_nav.js), unified card system (white/#E5E7EB border/16px radius/subtle shadow with hover lift), refined form inputs (#D1D5DB border/8px radius/blue focus ring), clean table headers (#F8FAFC bg, no uppercase), rounded buttons (10px radius, #1F4E8C primary). All scoped inside desktop media query — mobile untouched.
