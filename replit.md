...
- **Night Mode (Auto / Day / Night)**: A pure red-on-black theme for night sailing, activated by `html[data-theme="night"]` and controlled by a UI switch, with state persisting locally and optionally server-side. Critical Night CSS is inlined in the `<head>` of `layout.html` and `login.html` so the dark palette renders on first paint even before external stylesheets load. CSS link tags use `?v={{ asset_version }}` cache-busters derived from a content hash of the bundled stylesheets (see `asset_version.py`); `sw.js` uses a network-first strategy for `/static/*.css` so theme updates propagate on a single reload. A no-auth, no-cache `/diagnostics/theme?theme=night|light` page renders one of every UI primitive for instant visual smoke-testing.
- **Asset Cache-Buster Single Source of Truth**: `asset_version.py` hashes the seven tracked CSS files in `static/` and exposes `asset_version()` (used in templates as `{{ asset_version }}`) and `cache_name()` (used by the `/sw.js` route to substitute `__CREWLOG_CACHE_NAME__` in `static/sw.js` at request time). A CSS deploy automatically rotates both the page-level `?v=` query string and the service worker `CACHE_NAME` in lockstep — no hand-edited version constants. `tests/test_asset_version.py` pins down the contract.
- **Desktop Sidebar Breakpoint**: The off-canvas drawer becomes a permanent 220px left sidebar at `min-width: 1280px` (laptop). Below that, the hamburger and bottom tab bar are shown and the drawer is offscreen. The 1280px threshold is intentional — iPad Pro at 1024px landscape exposed a Chrome quirk that left a visible 220px gap. The canonical declaration lives in `static/ui_breakpoints.css` as `--cl-desktop-min` / `--cl-desktop-min-px`. `static/ui_nav.js` reads the value at runtime via `getComputedStyle(document.documentElement)` so JS never hardcodes the literal. `static/ui_ios_prime.css` and `static/ui_v1.css` still need the literal `1280px` in their `@media` rules (CSS media queries can't consume `var(...)`), but each block carries a header comment pointing back to `ui_breakpoints.css`. `tests/test_desktop_sidebar_breakpoint_single_source.py` parses the canonical value and asserts all four call sites agree; `tests/test_desktop_sidebar_breakpoint_e2e.py` runs Playwright at 768/1023/1024/1279/1280 px to catch any runtime regression.
