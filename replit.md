...
- **Desktop Sidebar Breakpoint**: The off-canvas drawer becomes a permanent 220px left sidebar at `min-width: 1280px` (laptop). Below that, the hamburger and bottom tab bar are shown and the drawer is offscreen. The 1280px threshold is intentional — iPad Pro at 1024px landscape exposed a Chrome quirk that left a visible 220px gap. The threshold lives in three files that must stay in sync: `static/ui_ios_prime.css` (`@media (min-width:1280px)` sidebar block), `static/ui_v1.css` (the `#drawer-open` hide rule), and `static/ui_nav.js` (`applyLayout()`). `tests/test_desktop_sidebar_breakpoint_e2e.py` runs Playwright at 768/1023/1024/1279/1280 px to catch any future drift.
