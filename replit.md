...
- **Night Mode (Auto / Day / Night)**: Red-on-black theme for night sailing, preserving dark adaptation. CSS-only overlay activated by `html[data-theme="night"]` (`static/ui_night_mode.css`). A 3-segment switch (Auto / Day / Night) is auto-injected into the desktop topbar and mobile drawer by `static/js/night-mode.js`; **Auto** follows `prefers-color-scheme: dark` and reacts live to system changes. State persists in `localStorage["crewlog-theme"]` AND server-side via `POST /api/preferences/theme` (CSRF-protected) which upserts `user_preferences.theme` (column added on startup via idempotent `ALTER TABLE … ADD COLUMN IF NOT EXISTS` in `db.init_db`). Server seeds the saved value into the layout via `saved_theme` context (legacy users only — SaaS users currently fall back to localStorage which works fine; the API still returns 200 for them). Inline early-paint script in `templates/layout.html` and `templates/login.html` `<head>` reads `localStorage > seeded > 'auto'` and resolves Auto via `matchMedia` before first paint to prevent white flash. Leaflet tile layers (`/templates/trip_track.html`) get a CSS `filter: brightness/contrast/hue-rotate/saturate` while in Night so the basemap dims down with the rest of the UI. User-uploaded photos/receipts (`/uploads/`, `.photo-thumb img`, `.receipt-thumb img`) get a dim red filter; icons and SVGs inherit color. i18n keys: `theme.label`, `theme.auto`, `theme.day`, `theme.night`. Service worker cache bumped to `crewlog-v21`.
