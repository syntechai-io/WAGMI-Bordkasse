...
- **Smart Dropdown Menus for Faster Entry**: Added datalist-based dropdown fields for wind direction (16-point compass rose), wind strength (Beaufort scale 0-12 with German descriptions), visibility (5 categories from sehr gut to sehr schlecht), and sail plan (vessel-specific: Motor, Großsegel, Genua, Großsegel + Genua, Keine Segel). These hybrid fields provide dropdown suggestions for quick selection while still allowing manual text entry for custom values. Preserves all existing legacy values and future-proofs for atypical entries.
- **Weather API Integration**: Added Open-Meteo API integration (free, no API key required) to auto-pull temperature, wind direction, wind speed (converted to Beaufort), and pressure from GPS coordinates. New "Wetterdaten automatisch abrufen" button fetches live weather data using coordinates from GPS button. Auto-sets weather source to "Open-Meteo API".
- **Comprehensive Testing**: Created test_weather_service.py (10 unit tests passing) for API parsing, Beaufort conversion, compass direction conversion, and error handling. Created test_regression_logbook.py (17 tests passing) to verify all existing features preserved: datalist dropdowns, weather API, GPS auto-pull, motor hours, PDF export, in-mast furling, watch leader, offline storage, and all Phase A fields.
