...
- **UI v1.2 Design System**: Calm, Apple-inspired marine design with `static/ui_v1.css` providing CSS custom properties (--bg, --surface, --surface-2, --border, --text, --text-secondary, --muted, --accent, --accent-2, --accent-light, --warning, --success, --warn, --danger) and reusable component classes (.btn, .btn-primary/secondary/danger, .card, .kpi-card, .tag, .dash-tile, .dash-hero, .topbar, .nav-item, .footer-bar). Dashboard cards use neutral white backgrounds with minimal accent (thin top border on KPI cards, no side borders). Icons muted (lower opacity, smaller size). Typography normalized: h1=1.75rem, h2=1.2rem, h3=1rem, line-height 1.35/1.55. Inter font throughout. Accent color from branding overrides --accent/--accent-2 only (not --navy-deep).
