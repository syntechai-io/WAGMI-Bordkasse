...
- **Desktop UI Refinement (Calm Maritime v2)**: Desktop-only (>=1024px) design system in `static/ui_desktop_skin.css`. Color variables: `--navy-shell` (#1E2F45), `--canvas-grey` (#F3F4F6), `--card-white`, `--text-primary`/`--text-secondary`/`--text-on-navy`, `--accent-blue` (#2563EB), semantic `--success-bg/text`, `--warning-bg/text`, `--danger-bg/text`. Thicker 64px topbar with premium navy feel, bottom-border active nav indicator (accent-blue 2px), unified card system (white/16px radius/soft shadow), 44px form inputs with 10px radius and blue focus ring, clean table headers (no uppercase, soft grey bg), 12px radius buttons (primary=navy-shell, secondary=white+border, danger=991B1B). `ui_navy_contrast_fix.css` guardrail with `.navy-shell` class ensures white text on navy header/footer with dropdown exclusions. All scoped inside desktop media query — mobile untouched. Service worker cache at v9.
