...
class TestBalanceCalculation:
    ...
    def test_simple_deposit_balance(self, db_session, test_trip, test_crew):
        ...
        assert alice_balance['deposits'] == 500.0
        assert alice_balance['net_balance'] == 500.0
    ...
    def test_private_expense_calculation(self, db_session, test_trip, test_crew):
        ...
        # Alice paid 100€ private and owes 25€ share = -75€ net
        assert alice_balance['private_paid'] == 100.0
        assert abs(alice_balance['share_owed'] - 25.0) < 0.01
        assert abs(alice_balance['net_balance'] - (-75.0)) < 0.01
...
class TestSettlementAlgorithm:
    ...
    def test_simple_settlement(self, db_session, test_trip, test_crew):
        ...
        balances = calculate_balances(db_session, test_trip.id)
        settlements = compute_settlement(balances)
        
        # Alice should receive from the other 3 (they each owe 100€)
        assert len(settlements) == 3
        for settlement in settlements:
            assert settlement['to_member_name'] == "Alice"
            assert abs(settlement['amount'] - 100.0) < 0.01
    ...
    def test_complex_settlement_minimizes_transfers(self, db_session, test_trip, test_crew):
        ...
        db_session.add(Deposit(trip_id=test_trip.id, member_id=test_crew[0].id, amount_eur=500.0, date=date(2025, 11, 1)))
        db_session.add(Deposit(trip_id=test_trip.id, member_id=test_crew[1].id, amount_eur=300.0, date=date(2025, 11, 1)))
